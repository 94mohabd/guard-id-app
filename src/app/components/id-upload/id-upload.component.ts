...
@Component({
  ...
  template: `
    <div class="container screen-enter">
      ...
      <div class="content">
        <h2>Upload Front<br>of Your ID</h2>
        <p>Make sure the photo is clear<br>and all details are visible.</p>
...
export class IdUploadComponent {
  ...
  async onFileSelected(event: Event) {
    ...
    if (element.files && element.files.length > 0) {
      ...
      try {
        const result = await this.imageVerificationService.processFrontIdImage(this.selectedFile);

        if (result.success) {
          this.previewUrl = result.previewUrl;
          this.selectedFile = result.resizedImage;
          this.isValidated = true;
          localStorage.setItem('frontId', result.previewUrl!);
        } else {
  ...
  uploadPhoto() {
    if (this.isValidated && !this.errorMessage) {
      this.router.navigate(['/id-upload-back']);
    }
  }
}